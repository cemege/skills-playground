...
**Location**: `skills/linear-task-planner/SKILL.md`

**Purpose**: Plan iOS implementation work from Linear boards, projects, or issues

**Use When**:
- You need to fetch Linear task details (title, description, attachments, design links, comments)
- You want a Markdown implementation plan for developers
- You want the plan to align with SwiftUI/concurrency/performance best practices
- You need to honor developer preference for API token vs MCP auth
- You need built-in guidance on obtaining Linear access tokens
- You want a starter env template (`.env.local.example`)
- You want `.env` / `.env.local` ignored by git
- You want a simple invocation like `$linear-task-planner SKI-1`
...
12. **Planning from Linear tasks** → Use `linear-task-planner`
